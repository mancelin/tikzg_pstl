...
Pour accélerer la génération du pdf, on supprimme l'affichage des messages de compilations de pdflatex en redirigeant la commande vers \textbf{/dev/null}
...
Ce script a un temps d'execution assez long, c'est à dire plus d'une vingtaine de minutes, mais n'a plus a être réexecuté, vu que le fichier produit \textbf{list\_IDC} est distribué avec notre logiciel.

Ce script génére un pour chacune des 5041 valeurs de couleur un n{\oe}ud coloré, le transforme en pdf, puis en png. La valeur de la couleur RGB est ensuite recupérée a des coordonnées données dans cette image, puis une nouvelle ligne est ajoutée a la liste en fonction du colorID courant et de la couleur RGB. 
